import { plannedItems } from "@/db/schema";
import { crudRoutes } from "@/lib/crud/resource";

export const dynamic = "force-dynamic";

const routes = crudRoutes({
  table: plannedItems,
  labels: {
    name: "název",
    amount: "částka",
    direction: "směr",
    interval: "opakování",
    month: "měsíc",
  },
  fields: {
    name: { type: "text", required: true },
    amount: { type: "money", required: true, min: 0 },
    direction: { type: "enum", required: true, values: ["income", "expense"] },
    interval: { type: "enum", required: true, values: ["once", "monthly"] },
    // Only 'once' items need a month; 'monthly' ones apply to every month.
    month: { type: "month", nullable: true },
    active: { type: "bool" },
  },
});

// Next discovers HTTP handlers by looking for these exact named exports,
// so they are written out rather than destructured off the factory result.
export const POST = routes.POST;
export const PATCH = routes.PATCH;
export const DELETE = routes.DELETE;
